...
def main():   
    ...
    df = pd.read_csv(r"D:\Projects\ETL\ETL\source_file\ml_sales\Warehouse_and_Retail_Sales.csv")    
    
    # Example: combine year and month into a datetime (first day of month)
    df['Date'] = pd.to_datetime(df['YEAR'].astype(str) + '-' + df['MONTH'].astype(str) + '-01')

    # Select only the required columns
    df_long = df[['Date', 'ITEM CODE', 'RETAIL SALES']]   

    df_long= df
    
    splits = {
        'df_long': df_long,
        'dimension_col': 'ITEM CODE',#'Material',
        'date_col': 'Date',
